...
config = load_config("./myconfig.ini")

# Use the configuration to initialize the Kraken exchange
...
INVESTMENT_AMOUNT_DOLLARS = 10000000
...
def write_to_csv(arbitrage_type, scrip1, scrip2, scrip3, profit_loss, orders, timestamp):
    with open('trade_history.csv', mode='a', newline='') as file:
        ...
        writer.writerow([f"{arbitrage_type}, {scrip1},{scrip2},{scrip3}, Profit/Loss: {round(profit_loss, 3)}", timestamp])
        for order in orders:
